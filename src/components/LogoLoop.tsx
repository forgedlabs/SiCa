'use client';
import { useCallback, useEffect, useMemo, useRef, useState, memo, type RefObject, type CSSProperties, type ReactNode } from 'react';
import './LogoLoop.css';
  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: string) => {
        if (renderItem) {
          return (
            <div className="logoloop__item" key={key} role="listitem">
              {renderItem(item, key)}
            </div>
          );
        }
        ...
        const content = isNodeItem ? (
          ...
          <img
            ...
            alt={item.alt ?? ''}
            title={item.title}
            loading={item.loading as "lazy" | "eager" | undefined || "lazy"}
            decoding={item.decoding as "async" | "auto" | "sync" | undefined || "async"}
            draggable={false}
          />
        );
